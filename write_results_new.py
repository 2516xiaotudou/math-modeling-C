...
# result1 长表标签：0:10-0:20 ... 0:00+1-0:10+1
LL1 = ['%s-%s' % (fm((j + 1) * 10), fm((j + 2) * 10)) for j in range(N)]
LL1[143] = '0:00+1-0:10+1'

# result2/3/4 宽表标签：0:10-0:20 ... 0:00-0:10+1
LW = ['%s-%s' % (fm((j + 1) * 10), fm((j + 2) * 10)) for j in range(N)]
LW[143] = '0:00-0:10+1'
...
def write_cd(ws, c_all, d_all, soc_traj, has_date=True, one_digit=False):
    """one_digit=True -> result1 用 '0:00'/'24:00'；否则用 '00:00'/'24:00'"""
    ws.append((['日期'] if has_date else []) + ['时间段', '充电量', '放电量', '时刻', '储电量'])
    t0 = '0:00' if one_digit else '00:00'
    t24 = '24:00'
    for di in range(ND):
        ...
        for k in range(6):
            ...
            if k == 0:
                tms = t0
                stg = round(float(soc[0]), 4)
            elif k == 1:
                tms = t24
                stg = round(float(soc[144]), 4)
            else:
                tms = None
                stg = None
...
br1 = rot(b1)
for j in range(N):
    ws.append([LL1[j], round(float(br1[j]), 4)])
...
# ---- result3 (4 sheets) ----
wb = Workbook(); ws = wb.active; ws.title = '计划购电量'
write_wide(ws, b_plan3, plan3_d)
ws2 = wb.create_sheet('调整购电量'); write_wide(ws2, b_adj3, plan3_d + adj3_d)
...
# ---- result4-3 (4 sheets) ----
wb = Workbook(); ws = wb.active; ws.title = '计划购电量'
write_wide(ws, b_plan43, plan43_d)
ws2 = wb.create_sheet('调整购电量'); write_wide(ws2, b_adj43, plan43_d + adj43_d)
